...
def data_prep(dataset, seed, mask_det=None, datasplit=[.65, .15, .2]):
    ...
    if dataset in ['1995_income','bank_marketing','qsar_bio','online_shoppers','blastchar','htru2','shrutime','spambase','arcene','creditcard','arrhythmia','forest','kdd99']:
        ...
        categorical_columns = []
        categorical_dims =  {} ## {1: 9, 3: 16, 5: 7, 6: 15, 7: 6, 8: 5, 9: 2, 13: 42, 14: 2, 'Set': 3}
    ...
    if mask_det is None:
    ...
    elif mask_det["method"] == "pt_mask_params":
        temp = temp[features]
        ...
        y_mask = np.zeros_like(train[target])
    elif mask_det["method"] == "pt_mask_con_cat" :
    ...
    # if train_type == "train":
    #
    #     howmany = int(howmany/2)
    #
    #     pos_loc = [index for (index,value) in enumerate(y_train['data']) if value == 1]
    #     neg_loc = [index for (index,value) in enumerate(y_train['data']) if value == 0]
    #     pos_loc = np.random.choice(pos_loc, howmany)
    #     neg_loc = np.random.choice(neg_loc, howmany)
    #     location = np.array(np.concatenate((pos_loc,neg_loc))).flatten()
    #     np.random.shuffle(location)
    #
    #     # selected_train_x = X_train['data'][location]
    #     # selected_train_x_mask = X_train['mask'][location]
    #     # selected_train_y = y_train['data'][location]
    #     # selected_train_y_mask = y_train['mask'][location]
    #
    #     X_train['data']= X_train['data'][location]
    #     X_train['mask']= X_train['mask'][location]
    #     y_train['data']= y_train['data'][location]
    #     y_train['mask']= y_train['mask'][location]




    train_mean, train_std = np.array(X_train['data'][:,con_idxs],dtype=np.float32).mean(0), np.array(X_train['data'][:,con_idxs],dtype=np.float32).std(0)
    return cat_dims, cat_idxs, con_idxs, X_train, y_train, X_valid, y_valid, X_test, y_test, train_mean, train_std, train_size

def partical_train_data(seed,X_train, y_train ,con_idxs, howmany):
    np.random.seed(seed)
    howmany = int(howmany / 2)
    ...
    return X_train, y_train,train_mean,train_std
